  ...
  _checkShowMenu = (item) => {
    ...
    try {
      ...
      if (CurrentPermission === CONSTANTS.SUPER.roles) {
        ...
        let MaQuyenHienThi = item.Ma.map((qht) => {
          return qht.Ma;
        });
        ...
        let checkQuyenHienThi = cmFunction.findCommonElements2(
          MaQuyenHienThi,
          MaQuyen
        );
        ...
        {array.map((item, index) => {
          ...
          if (checkShowMenu) {
            ...
            let checkActive =
              ...
              router.findIndex((x) => `/${x}` === item.URL) !== -1;
            return (
              <React.Fragment key={index}>
                <Link
                  to={item.URL || "#"}
                  className={
                    (hasChildren ? "nav-link collapsed" : "nav-link") +
                    (checkActive ? " active" : "")
                  }
                ...
                {hasChildren ? (
                  <div
                    className={`collapse margin-left-${level}x`}
                    id={target + index}

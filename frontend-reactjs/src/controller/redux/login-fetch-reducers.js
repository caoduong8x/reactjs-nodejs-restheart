...
export function requestLoginReducer(LoginReq = {}, action) {
  switch (action.type) {
    case FETCH_LOGIN_REQUEST:
      console.log("5. requestLoginReducer");
...
export const fetchLoginEpic = (action, store) =>
  action.ofType(FETCH_LOGIN_REQUEST).mergeMap(() => {
    return axios
      .post(`${HOST_API}${AUTH_SIGNIN}`, store.getState().LoginReq)
      .then((res) => {
        if (res.data) {
          console.log("6. fetchLoginEpic");
...
export function fetchLoginReducer(LoginRes = {}, action) {
  switch (action.type) {
    case FETCH_LOGIN_SUCCESS:
      ccFunction.setCookie(
        AUTHORIZATION_COOKIE,
        cmFunction.encode(JSON.stringify(action.payload))
      );
      console.log("7. fetchLoginReducer: ", action.payload);
      return action.payload;

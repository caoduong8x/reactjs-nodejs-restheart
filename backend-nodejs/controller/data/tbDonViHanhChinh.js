    ...
    .catch(function (rhApiErr) {
      try {
        mwLog.updateLogApi(req, {
          status: rhApiErr.response.status,
          body: rhApiErr.response.data,
        });
...
async function post(req, res) {
  ...
  let checkIsEx = await checkIsExisted("tbDonViHanhChinh", "Ma", req.body.Ma);
  if (checkIsEx) {
    mwLog.updateLogApi(req, {
      status: constRes.RESPONSE_ERR_CODE_EXITSTED.status,
      body: constRes.RESPONSE_ERR_CODE_EXITSTED.body,
    });
    res
      .status(constRes.RESPONSE_ERR_CODE_EXITSTED.status)
      .send(constRes.RESPONSE_ERR_CODE_EXITSTED.body);
    return;
  } //Hết kiểm tra
  ...
  // return
  let rhApiUrl = _configs.rh.dataUrl + "/tbDonViHanhChinh" + req.url;
  Axios("post", rhApiUrl, req.body) //method, url, data
    .then(function (rhApiRes) {
      if (rhApiRes.headers.location) {
        rhApiRes.data = {
          newId: rhApiRes.headers.location.substr(
            rhApiRes.headers.location.lastIndexOf("/") + 1
          ),
        };
      }
      mwLog.updateLogApi(req, { status: rhApiRes.status, body: rhApiRes.data });
...
function put(req, res) {
  ...
  let rhApiUrl = _configs.rh.dataUrl + "/tbDonViHanhChinh" + req.url;
  Axios("put", rhApiUrl, req.body) //method, url, data
    .then(function (rhApiRes) {
      mwLog.updateLogApi(req, { status: rhApiRes.status, body: rhApiRes.data });
      res.status(rhApiRes.status).send(rhApiRes.data);
    })
...
function patch(req, res) {
  ...
  Axios("patch", rhApiUrl, req.body) //method, url, data
    .then(async function (rhApiRes) {
      //update multi
      await updateMulti(
        "tbUsers",
        { "DonVi.code": body.code },
        { DonVi: body }
      );
...
function deleteTable(req, res) {
  let rhApiUrl = _configs.rh.dataUrl + "/tbDonViHanhChinh" + req.url;
  Axios("delete", rhApiUrl) //method, url, data
    .then(function (rhApiRes) {
      mwLog.updateLogApi(req, { status: rhApiRes.status, body: rhApiRes.data });
      res.status(rhApiRes.status).send(rhApiRes.data);
    })
...
function lock(req, res) {
  ...
  Axios("get", rhApiUrl) //method, url, data
    .then(function (rhApiRes) {
      Axios("patch", rhApiUrl, { isActive: false }) //method, url, data
        .then(function (dataApiRes) {
          mwLog.updateLogApi(req, {
            status: dataApiRes.status,
            body: dataApiRes.data,
          });
